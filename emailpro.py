...
T_DATA = {
    "pt": {
        "app_title": "CorreioPro Ultimate", "pgp_your_name": "Seu Nome (para a chave)", "pgp_your_email": "Seu Email (associado à chave)",
    ...
    "en": {
        "app_title": "CorreioPro Ultimate", "pgp_your_name": "Your Name (for the key)", "pgp_your_email": "Your Email (linked to the key)", "pgp_passphrase": "Private Key Passphrase", "login_title": "MAIL PRO", "connect": "Login", "manage_profiles": "Profiles", "about": "About", 
...
class EmailBackend:
    ...
    def send(self, to, sub, body, atts=[], request_receipt=False):
        try:
            m = MIMEMultipart()
            m['From'] = self.c['email']; m['To'] = to; m['Subject'] = sub; m['Date'] = formatdate(localtime=True); m['Message-ID'] = make_msgid()
            if request_receipt: m['Disposition-Notification-To'] = self.c['email']; m['Return-Receipt-To'] = self.c['email']
            m.attach(MIMEText(body, 'plain'))
            for p in atts:
                with open(p, "rb") as f:
                    part = MIMEBase("application", "octet-stream"); part.set_payload(f.read())
                encoders.encode_base64(part)
                # FIX: Changed "CD" to "Content-Disposition" so Gmail/Outlook recognize it
                part.add_header("Content-Disposition", f"attachment; filename={os.path.basename(p)}")
                m.attach(part)
            s = self.conn_smtp()
            s.sendmail(self.c['email'], to, m.as_string())
            s.quit()
            try:
...
class DarkMailApp:
    ...
    def open_compose(self, to="", sub="", body="", atts=[]):
        ...
        # --- BOTTOM ACTION BAR ---
        bot_frame = tk.Frame(f, bg=BG_DARK, pady=10)
        ...
        ttk.Button(bot_frame, text=T("attach"), command=add, width=10).pack(side=tk.LEFT)
        l_a.pack(side=tk.LEFT, padx=10)
        ...
        def snd(): 
            ...
            if v_pgp.get():
                ...
                if enc_body: 
                    ...
                    messagebox.showerror(T("error"), "PGP Encryption Failed.\n(Check key validity)", parent=w)
                    return
    ...
    def delete(self):
        s = self.tree.selection()
        if s and messagebox.askyesno(T("confirm"), T("delete_confirm")): 
            threading.Thread(target=lambda: self.back.delete(self.v_fld.get(), self.emails[int(s[0])]['id']) and self.refresh(), daemon=True).start()
